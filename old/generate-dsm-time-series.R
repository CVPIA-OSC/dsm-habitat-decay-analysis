...
calib_scaledown_1 <- 0.21
calib_scaledown_2 <- 0.1
calib_scaledown_3 <- 0.05
...
# flow to sediment transport curve 
flow_cfs_to_sed_cfd_calibrated <- approxfun(
  x = rating_curve$flow_cfs, 
  y = rating_curve$sed_ft3_per_day_min * rep(calib_scaledown_1, 
...
# raw decay plots 
upper_sac_decay |> 
  ggplot(aes(date, decay_acres_month)) + geom_line()

upper_sac_decay |> 
  filter(year(date) == 1985) |> 
  ggplot(aes(date, decay_acres_month)) + geom_line() + geom_point()

write_csv(upper_sac_decay, "data/prop-of-habitat-decay.csv")
...
decays_and_augs <- decays |> 
  ...
  mutate(aug_acres = ifelse(is.na(aug_acres), 0, aug_acres), 
         ...
         aug_minus_decay_scaled = aug_minus_decay * .21,
         month = month(date),
         year = year(date))
...
decays_and_augs |> 
  ...
  ggplot(aes(year, -avg_diff)) + geom_col() + 
  geom_hline(yintercept = 0, size=1) + 
  geom_hline(yintercept = 2.44297, linetype = 2, color = "blue", size = 1) + 
...
decays_and_augs |> 
  ...
  ggplot(aes(year, -avg_diff)) + geom_col() + 
  geom_hline(yintercept = 0, size=1) + 
  geom_hline(yintercept = 0.5130238, linetype = 2, color = "blue", size = 1) + 
  scale_x_continuous(breaks = 1979:2000)

mean(decays_and_augs$aug_minus_decay)
...
decays_and_augs |> 
  ...
  labs(y = "Augmentation minus Decay")

decays_and_augs |> 
  ggplot(aes(date, aug_minus_decay_scaled)) + geom_line() + 
  labs(y = "Augmentation minus Decay")
...
decays_and_augs |> 
  ggplot(aes(date, decayed_habitat)) + geom_line() +
  labs(y = "Acres")
